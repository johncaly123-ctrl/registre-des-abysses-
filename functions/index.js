// Portage Cloudflare Pages Functions de netlify/edge-functions/og-partage.js
// (même logique, API légèrement différente : ici on doit toujours retourner
// une Response, y compris pour laisser passer la requête via context.next()).
...
// avec le nom de l'éleveur plutôt que le titre générique du site.
export async function onRequestGet(context) {
  const { request } = context;
  const url = new URL(request.url);
  const pseudo = url.searchParams.get("voir");
  if (!pseudo) return context.next();
