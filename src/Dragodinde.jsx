...
function plierCouleurDragodinde(value) {
  return String(value || "")
...
function DragodindeMiniCard({ m, selected, onClick }) {
    ...
    <div onClick={onClick} className="panel-card" style={{ padding: 10, marginBottom: 8, cursor: "pointer", border: selected ? "1px solid var(--gold)" : sterile ? "1px solid rgba(232,137,106,.4)" : "1px solid var(--line)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontWeight: 700, fontSize: 13, display: "inline-flex", alignItems: "center", gap: 6 }}>
          {sexeDragodinde(m) === "F" ? "♀" : sexeDragodinde(m) === "M" ? "♂" : "?"} <DragodindeBadge couleur={m.couleur} taille={16} /> {m.nom || m.couleur}
        </span>
...
function NewDragodindeModal({ onClose, onCreate }) {
  const [form, setForm] = useState({ nom: "", couleur: COULEURS_DRAGODINDE[0], sexe: "Femelle", statut: "Fertile" });
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 80, background: "rgba(10,8,6,.65)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div onClick={(e) => e.stopPropagation()} className="panel-card" style={{ width: "min(420px,92vw)" }}>
        <h3 style={{ marginTop: 0 }}>Nouveau dragodinde</h3>
        ...
        <label style={{ fontSize: 11, color: "var(--muted)", display: "block", marginTop: 8 }}>Couleur
          <select className="field" value={form.couleur} onChange={(e) => setForm({ ...form, couleur: e.target.value })}>
            {COULEURS_DRAGODINDE.map((c) => <option key={c} value={c}>{c} (G{generationDeCouleurDragodinde(c)})</option>)}
          </select>
        </label>
...
export function DragodindeCheptelMainPane({ cheptel, selectedId, setSelectedId, byId, onPatch, onDelete, showNew, setShowNew, onCreate, onVoirGenealogie }) {
  ...
  return (
    <>
      ...
      {showNew && <NewDragodindeModal onClose={() => setShowNew(false)} onCreate={onCreate} />}
    </>
  );
}
...
export function DragodindeSuccesPage({ historiqueCouleurs, cheptel, onToggleCouleur, onValidateGeneration }) {
  const presentes = new Set(cheptel.map((m) => m.couleur));
