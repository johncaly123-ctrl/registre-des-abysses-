...
export function DragodindeBadge({ couleur, taille = 22 }) {
...
function RechercheDragodindeDeroulante({ muldos, valeurId, onChoisir, placeholder, exclureId, onEffacer }) {
  ...
  return (
    <div className={`muldo-card ${ready ? "muldo-ready" : ""} ${sterile ? "muldo-sterile" : ""} ${(modeSelection ? coche : selected) ? "muldo-selected" : ""}`} onClick={onClick}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
        ...
        <span className="pill">G{generationDeCouleurDragodinde(m.couleur)}</span>
      </div>
...
// seule) et DragodindeCheptelOverviewPage (recherche + filtres en cartouche).
export function filtrerCheptelParTexteDragodinde(cheptel, filter) {
...
export function DragodindeDetail({ m, byId, onPatch, onDelete }) {
  const action = getNextActionDragodinde(m);
              ...
              <div key={index}>
                ...
                {parentReel ? (
                  ...
                  <RechercheCouleurDeroulante
                    couleurs={couleursGenerationJusquaDragodinde(10)}
                    valeur={m.parentsCouleurs?.[index] || null}
                    placeholder="Couleur…"
                    plierCouleurFn={plierCouleurDragodinde}
                    generationFn={generationDeCouleurDragodinde}
...
export function DragodindeCheptelOverviewPage({ cheptel, filter, setFilter, selectedId, setSelectedId, onSupprimerPlusieurs, onMarquerStatutPlusieurs }) {
  ...
  const parTexte = useMemo(() => filtrerCheptelParTexteDragodinde(cheptel, filter), [cheptel, filter]);
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "center", marginBottom: 18 }}>
        <div>
          ...
          <h1 style={{ margin: "6px 0 0", fontSize: 34 }}>Cartes de dragodindes</h1>
        </div>
      ...
      {modeSelection && (
        <div className="panel-card" style={{ marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <span style={{ fontSize: 13, color: "var(--muted)" }}>{idsSelectionnes.size} dragodinde(s) sélectionné(s) — clique sur une carte pour la cocher/décocher.</span>
      ...
      <div className="panel-card" style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 10 }}>
          <LabeledSelect
            ...
            options={[["", "Toutes"], ...Object.keys(GENERATIONS_DRAGODINDE).map((g) => [g, `Génération ${g}`])]}
          />
          <LabeledSelect
            label="Couleur"
            value={filtreCouleur}
        ...
        {filtresActifs && (
          <div style={{ marginTop: 10, fontSize: 12, color: "var(--muted)" }}>{cheptelFiltre.length} / {cheptel.length} dragodinde(s) affiché(s)</div>
        )}
      </div>
