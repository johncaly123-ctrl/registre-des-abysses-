...
function scoreCoupleObjectifVolkorne(male, femelle, objectif, distances, purification = false) {
  ...
  score += Math.max(0, 20 - collisionScoreVolkorne(male, femelle, {}));
...
function optimiserSessionAccouplementsVolkorne(cheptel, objectif, purification = false) {
  ...
  while (malesRestants.length && femellesRestantes.length) {
    const details = malesRestants.map((male) => femellesRestantes.map((femelle) => scoreCoupleObjectifVolkorne(male, femelle, objectif, distances, purification)));
...
export function VolkorneGpsPage({ cheptel, objectif, setObjectif, generationCible, setGenerationCible, purification, setPurification, byId, historiqueCouleurs, onRealiserUn }) {
  const session = useMemo(() => optimiserSessionAccouplementsVolkorne(cheptel, objectif, purification), [cheptel, objectif, purification]);
      ...
      <div className="panel-card" style={{ marginTop: 16 }}>
        ...
        {session.groupes.map((g) => (
          <div key={g.key} style={{ padding: "10px 0", borderTop: "1px solid rgba(255,255,255,.06)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
            <span><b>× {g.quantite}</b>{" "}♂ <VolkorneBadge couleur={g.male.couleur} taille={16} /> {g.male.couleur} × ♀ <VolkorneBadge couleur={g.femelle.couleur} taille={16} /> {g.femelle.couleur}</span>
